...
//  --------------------------------------------------
    ...
    //!!! оператор == нужно реализовывать отдельно
    //так как считается что логика его работы может быть проще и быстрее чем сравнение на < >

    //??? куда то пропала default реализация <=>
/*
    friend auto operatir<=>(Entity_v2 &lhs, Entity_v2 & rhs) = default;
    она будет работать точно также как и реализация выше
    + произведется генерация кода для оператора == (без использования <=>)
*/
